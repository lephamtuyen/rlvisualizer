    ...
    def train(self):
        ...
        while episodes < self.max_episode:
            ...
            if self.is_plot:
                ...
                if "Ship" in self.env_name:
                    trajectory.append([self.env.env.get_xhist(), self.env.env.get_yhist()])
            ...
            if self.is_plot and episodes % self.save_plot_interval == 0:
                ...
                if "Bicycle" in self.env_name:
                    if "Bicycle-v0" in self.env_name:
                        self.save_plot_data("back_trajectory", np.asarray([back_trajectory, [None, 5]]),
                                            is_train=True)
                        self.save_plot_data("front_trajectory", np.asarray([front_trajectory, [None, 5]]),
                                            is_train=True)
                    else:
                        self.save_plot_data("back_trajectory", np.asarray([back_trajectory, [np.array([self.env.env.x_goal,self.env.env.y_goal]), 5]]), is_train=True)
                        self.save_plot_data("front_trajectory", np.asarray([front_trajectory, [np.array([self.env.env.x_goal,self.env.env.y_goal]), 5]]), is_train=True)

                if "Ship" in self.env_name:
                    self.save_plot_data("trajectory", np.asarray([trajectory, [self.env.env.goal, 5]]), is_train=True)
            ...
            if "Bicycle" in self.env_name:
                if "Bicycle-v0" in self.env_name:
                    ...
                    self.save_plot_data("back_trajectory", np.asarray(
                        [back_trajectory, [np.array([self.env.env.x_goal, self.env.env.y_goal]), 5]]), is_train=True)
                    self.save_plot_data("front_trajectory", np.asarray(
                        [front_trajectory, [np.array([self.env.env.x_goal, self.env.env.y_goal]), 5]]), is_train=True)
